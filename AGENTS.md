...
- QML 错误只在运行时暴露，改完 QML 应按「静态 → 无头 → 起进程」三步验证：① 静态扫描 `qmllint.exe -I D:/Qt/6.8.3/msvc2022_64/qml <全部 .qml>`（能报 `Label is not a type`、`Cannot assign to non-existent property` 这类硬错误；`QWindowKit` 模块解析失败属正常误报——该模块由 CMake target 注册、无 qmldir）；② 无头加载验证用 `QT_QPA_PLATFORM=offscreen qml.exe <probe.qml>`，probe 需放在 `Chat-Client/resources/` 下才能解析相对 `import`，**不要**写进 `resources.qrc`，验完即删；③ 最终以 `QT_QPA_PLATFORM=offscreen timeout 12 ./Chat-Client.exe` 输出中不出现任何 `.qml` 行与 `TypeError`/`ReferenceError` 为准（该 exe 为 console 子系统，stderr 可直接落盘）。④ 只在使用时才触发的运行期告警（如信号处理器隐式参数注入），"加载但不触发"的 probe 抓不到，需用 Qt Quick Test 打真实按键：`QT_QPA_PLATFORM=offscreen QT_QUICK_CONTROLS_STYLE=Basic qmltestrunner.exe -input <tst_*.qml>`（`tst_*.qml` 同样放 `Chat-Client/resources/` 以解析 `import "components"`，验完即删）；注意 `keyClicks` 不存在，`keySequence()` 只接受 `"Ctrl+A"` 这类键位串（传裸文本会 assert 在 `qasciikey.cpp`），逐字符输入用 `keyClick(Qt.Key_A + (c - 97))`。断言须先用 `git checkout HEAD -- <文件>` 回退到改动前跑一遍复现，确认用例真能抓到缺陷。`qmltestrunner` 的 stdout 同样**被吞掉**（同 ⑥），结果须用 `-o <file>,txt` 落盘读取；用例与探针副本可放在 `out/` 下（组件与 `theme/` 分别复制到 `out/probe/`、`out/theme/`，用例放 `out/tst_*.qml` 并 `import "probe"`）——目录 import 不要求产物进 `resources.qrc`，比放进 `Chat-Client/resources/` 更干净。⑤ **`qml.exe` 无法加载应用的 qrc**（没有 `--resource`/`-R` 选项），而 `Icon.qml` 的 `source` 硬编码 `qrc:/icons/*.svg`，直接拿生产组件做 probe 会报 `Cannot open: qrc:/icons/x.svg` 且图标全空白。可靠做法：把要验的组件（含 `Icon`/`LoadingIndicator`/`AppButton`/`Avatar`…）与 `theme/`（`Theme.qml` + `qmldir`）复制到 `out/probe/` 与 `out/theme/`（同目录类型隐式可见、`import "../theme"` 照常解析），仅用 `sed 's|qrc:/icons/|file:///…/resources/icons/|'` 改这一处 URL，`diff` 确认无其他差异后再跑；这样测的着色逻辑与生产逐字相同。⑥ **本环境 `qml.exe` 的 stdout 会被吞掉**（`console.log` 全丢，日志为空但进程 exit 0），要拿到数值证据就把表达式渲染进界面（图标底下铺一个 `Rectangle` 放 `Label` 显示 `contentWidth=f.xxx`），再从抓图里读。⑦ **`grabToImage()` 抓 `contentItem` 不含窗口自身背景**（结果背景透明、按 RGB 读出来是白/黑），白色图标在"白底"上会看不见、也易把透明区的 `rgb(0,0,0)` 误判成"图标是黑的"；务必在被测控件下铺真实底色（如 `color: Theme.sidebarBackground`），且分析 PNG 时**按 alpha > 200 过滤出实心像素再取色**，否则结论完全不可信。⑧ `layer.effect: MultiEffect` **是**官方支持写法（Qt 自身文档注释即此例），排查着色问题不要往"这个写法不生效"上猜。⑨ 用 `qml.exe` 做 probe **必须显式指定样式**：应用在 `main.cpp` 里 `QQuickStyle::setStyle("Basic")`，而 `qml.exe` 取平台默认样式，两者的几何与调色板并不相同，漏写会得到与生产无关的结论。本次排查"菜单中文标签被省略号截断"就是纯假象——非 Basic 样式的 `T.Menu` 默认带 **−32 的负 inset**，而 `implicitWidth: implicitBackgroundWidth + leftInset + rightInset` 会把 popup 宽从 168 削到 104（`item` 只有 94 宽 → 标签 42px 装不下 52px 文本），Basic 下 insets 为 0、几何完全正确。凡 probe 涉及 Controls 控件（`Menu`/`Button`/`Dialog`/`TextField`…），命令一律写全 `QT_QUICK_CONTROLS_STYLE=Basic qml.exe <probe.qml>`，并在下结论前先确认样式一致。⑩ 从抓图读结论时不要靠肉眼辨符号与像素，把判定直接渲染进界面：本次把"`btnX < 0` 才算气泡左侧外侧"封装成 `OUTSIDE-LEFT/RIGHT(…px gap)` / `INSIDE overlap=…px` 串，否则 10px 字号下的负号会被看漏（把 −74 读成 74，结论正好相反）。暴露几何用只读属性（`readonly property real probeBtnX: actionButtons.x`），并始终用 `diff --strip-trailing-cr <生产文件> out/probe/<同名>` 证明探针副本与生产**只有**计划内的那几处差异（本次为：图标 `qrc:/`→`file:///`、几何暴露属性、`rowHover.hovered` 强制 true 以在无头抓图里显示按钮）；`AppMenu.qml`/`AppMenuItem.qml` 这类不含图标 URL 的组件应做到 diff 为空。
- **`QNetworkReply` 的 `HttpStatusCodeAttribute` 读出 0 时，先查 URL 是否缺 host**（2026-09-17 教训）：`TestFileHttpService` 的 `get`/`put` 必须传完整 `m_base + "/..."`。传裸路径 `/file/1` 时 `QUrl` 是**相对** URL、无 host，请求在发起前即失败，得到的是 status **0** 而非任何 HTTP 码——极易误判为服务端缺陷（本次一度如此）。
- **用"把 `expires_at` 拨到只剩 N 秒"构造"即将过期"时，N 要留足**（2026-09-17）：用 1 秒会让"SQL 更新 → 随后的 HTTP/续期调用"之间的一次调度停顿就把票据变成真的过期，得到**用例自身的假失败**。改用 10 秒并断言"续期后回到完整窗口"即可既稳定又能区分续期与未续期。
- **局部对象按声明逆序析构**：被 lambda 按引用捕获的计数器/标志（`int count; bool flag;`）要声明在 `FileTransferManager engine;` **之前**，这样连接随 engine 先销毁，不会在析构尾声中引用已亡变量。
- **验证"上限/封顶"类断言时，破坏方式要选对**（2026-09-17）：要证伪 `min(a, b)` 的封顶断言，应把 `min` 改成 `max`；若改成只保留 `a`（`datetime('now', ?)`）反而**不会**被现有断言抓到，因为此时结果仍可能落在上限之内。同类"看起来等价"的破坏要挑真正消除该不变量的那种。
...
- 携密钥的结构（如文件清单）必须有**唯一脱敏出口**，且每一条通向 UI/JS 的路径都经它：只在一条路径（如实时推送）上脱敏等于没做——离线补收、历史翻页、本地缓存回填同样会把密钥渲染上屏，而字符串一旦进入 JS 堆就无法可靠清零。出口处再加一道“形态像就置空”兜底，使新增出口不会重蹈覆辙。同理，**把正文转成会话预览文本**也有两条路径（`NetworkManager::conversationPreviewFor` 走服务端下发与实时推送、`LocalStore::loadConversations` 走本地缓存回填），映射实现只保留 `Protocol::filePreviewText` 一处，由 `TestFileProtocol::filePreviewTextHidesKeyMaterial` 锁住“降级时也只出 `[File]`、绝不回退成清单原文”。
- 服务端 fan-out 明确排除发送者（`RequestHandler` 的 `if (memberId != operatorId)`），故**发送者自己的消息必须由客户端回显**，且两类消息的回显路径不同：文本消息在 QML 侧乐观插入（`appendOptimisticMessage`）；文件消息**做不到乐观插入**——清单要等 hashing 与元数据提取完成才存在，QML 事先拿不到 sha256/尺寸/缩略图。因此文件消息走“发送确认后回显”：`NetworkManager` 在 `sendMessageRequested` 时把明文清单记入 `m_pendingFileSends`，`handleSendMessageResponse` 成功后由 `echoSentFileMessage` 经 `sanitizeForUi`/`attachFileInfo` 补登记清单（`registerIncomingFile`）并 `emit fileMessageSent`，QML 侧 `onFileMessageSent` 插入气泡。新增“发送后要立即看到”的行为时，先判断该类型消息能否乐观插入，不能就必须补一条回显路径，否则表现为“要手动刷新才出现”。
- 本 app 固定使用 **Basic** 样式，而 Controls 控件的 `palette` 取的是**系统**调色板、与 `Theme` 无关：暗色主题下裸 `Menu` 是一块白面板、`MenuItem` 的 `icon.color`（走 `IconLabel`）让白图标落在白底上几乎不可见，`MenuSeparator` 取 `palette.mid` 是一条亮线。弹出菜单一律用 `AppMenu`/`AppMenuItem`（面板/边框/圆角/文字/图标/高亮/分隔线全部走 `Theme`，图标复用 `Icon` 以共用 `MultiEffect` 的 `brightness + colorization` 着色口径）；新增任何“裸 Controls 控件”前先确认它在暗色主题下的取色来源。同理，`Menu` 的宽度靠 `background.implicitWidth` 支撑（其 contentItem 是 `ListView`，只声明 `implicitHeight`），不要指望菜单项的 `implicitContentWidth` 撑开面板。
...
- Qt 6 QML 必须用 Qt 6 的 API，写成 Qt 5 的写法会在运行时以“编译期”错误暴露：① 控件（`Label`/`Button`/`Dialog`/`TextField` 等）来自 `QtQuick.Controls`，只 `import QtQuick` 会报 `Label is not a type`；② 图标着色用 `QtQuick.Effects` 的 `MultiEffect`，Qt 5 的 `ColorOverlay.colorizationStrength` 在 Qt 6 不存在，且 `colorization` 是 0~1 的 real 而**不是**颜色；**⚠️ 但只写 `colorizationColor: <颜色>` + `colorization: 1.0` 会把图标渲染成黑色**——`ColorOverlay` 走 `QPainter::CompositionMode_SourceIn`（**替换**颜色），而 `MultiEffect.colorization` 是**乘法**着色：`color.rgb = gray * colorizationColor.rgb * colorizationColor.a + color.rgb * (1 - colorizationColor.a)`（`qtdeclarative/src/effects/data/shaders/multieffect.frag`）。本仓图标源绝大多数是黑色描边（`gray == 0`），乘任何颜色仍是黑，于是暗色主题下"图标没切白"只是表象，根因是着色根本不生效。**必须在同一个 `MultiEffect` 上补 `brightness: 1.0`**（同文件第 82 行 `color.rgb += brightness * color.a` 先于 colorization 执行）把源抬成白色剪影再着色，才能得到与 `iconColor` 完全一致的颜色；因乘了 alpha，透明区域仍透明、抗锯齿边缘过渡不变。正确范式即 `components/Icon.qml`；③ 内边距命名为 `padding`/`topPadding`/`bottomPadding`/`leftPadding`/`rightPadding`，**没有** `paddingBottom`；④ 信号处理器**禁止**依赖隐式参数注入——需要事件对象必须写带形参的箭头函数：`Keys.onReturnPressed: (event) => { ... }`，写成 `Keys.onReturnPressed: { ... event ... }` 会报 `Parameter "event" is not declared. Injection of parameters into signal handlers is deprecated`（同为 `qt.qml.context` 类别，运行期输出，`qmllint` 归类为 `[unqualified]`）。
