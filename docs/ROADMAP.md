...
- 新增 `Packet` 数据结构与 `PacketCodec`。
- 实现长度前缀帧协议，禁止直接假设一次 `readyRead` 就是一个完整 JSON。
- 增加协议版本字段。
- 增加请求 ID、响应 ID、统一错误码。
- 客户端 `NetworkManager` 改为连接状态机：未连接、连接中、已连接、登录中、已认证、断线重连。
- 服务端 `RequestHandler` 改为可处理多个连续包。
- 增加 ping/pong 心跳与空闲超时。

### 验收标准

- 连续发送 1000 个小包不会解析错乱。
- 单个大包拆成多次到达时仍可正确解析。
- 客户端能根据 `requestId` 匹配登录响应。
