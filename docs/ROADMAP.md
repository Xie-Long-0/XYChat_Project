...
| M10 | UI 重构与体验完善（保守范围） | 已完成 | 2026-09-14 | **保守主体 + 精选新能力**：Phase 0-2 纯 QML 重构（Theme token 分层、SVG 图标库 + `Icon` 组件、基础组件库 `AppButton`/`AppTextField`/`AppDialog`/`Avatar`/`Toast`/`EmptyState`/`LoadingIndicator`/`NetworkStatusBar`、全局反馈层、`MainPage` 拆分为 9 个独立 Dialog、加载指示、搜索体验、Avatar 接入、消息气泡重构、未读分隔线 + 跳到底部 FAB、输入区多行重构、会话右键菜单）；Phase 3 两项新协议（会话整表删除 `100-102` + “正在输入”`103-105`）；Phase 4 六项历史欠账清理（传输横幅多任务 `Repeater`、O(n) 扫描改 Map 索引、`saveToFile` 异步化、上传 hashing 异步化、文件消息重发引导、Theme 装饰性注释）。`ctest` 12/12、`qmllint` 零错误。**原 M10 任务中的本地消息搜索/系统托盘/桌面通知/草稿/emoji 选择器/设置页/响应式布局/国际化经用户确认明确排除**（保守范围），见 §4.2 |
...
| 多端同步 | 账号级 `sync_events` 事件流（message/receipt/contact_added/group_changed/conversation_prefs/message_edited/message_deleted/read_cursor）+ 设备本地游标，登录后缓存先行 + 增量拉取（hasMore 自动续拉）；已读状态、会话偏好、消息编辑/删除均多端一致（实时推送 + sync_events 兜底）。**未实现**：会话整表删除已于 M10（2026-09-14）实现（类型 `100-102` + `conversation_deleted` 事件，硬删除并通知全体前成员，群聊仅群主可删） |
| 传输安全 | TLS 1.2+ fail-closed（服务端无证书拒启、客户端无 CA 拒连，开发明文需显式开关）；重放保护（timestamp ±300s + nonce 全局 TTL 600s 去重）；日志脱敏（LogSanitizer）；结构化日志（StructuredLogger 单行 JSON，M11 前置）；发消息/搜索/密钥拉取/编辑删除/会话偏好连接级限流（RateWindow；`send_message` 30/10s、`search_users` 20/60s、edit/delete 共用 20/60s、prefs 30/60s、`fetch_keys` 60s/20、“正在输入”10/10s（M10））；编辑/删除事件专用推送类型 `MessageEditedNotification (88)`/`MessageDeletedNotification (89)`；会话删除/正在输入类型 `100-105`（M10） |
| 客户端 UI | QML/Qt Quick + QWindowKit 无边框双窗口（登录/主窗口独立）；Telegram 风格主题（亮/暗切换持久化）；**M10（2026-09-14）视觉系统与组件库**：Theme token 分层（elevation/状态色/`avatarColor(id)` 确定性配色/字号 scale/动画曲线）、SVG 图标库 + `Icon.qml`（MultiEffect 着色，替换 Canvas 手绘与 emoji）、基础组件库（`AppButton`/`AppTextField`/`AppDialog`/`Avatar`/`Toast`/`EmptyState`/`LoadingIndicator`/`NetworkStatusBar`）、全局反馈层（顶层 Toast + 网络状态条，失败信号全部经 Toast）、`MainPage` 拆分为 9 个独立 Dialog（`resources/dialogs/`）、消息气泡重构（SVG 矢量状态勾、failed 点击重发、hover 编辑/删除/复制、文本可选中）、未读分隔线 + 跳到底部 FAB、输入区多行 `TextArea`（Enter 发送/Shift+Enter 换行/字符计数器）、“正在输入…”头部指示；会话右键菜单（置顶/免打扰/标记已读/删除会话）与消息右键菜单（编辑/删除/复制） |
| 文件与媒体传输 | **M8.1 地基（2026-09-10）+ M8.2 数据面与客户端 + M8.3 多媒体元数据与播放器（均 2026-09-11）**：控制面走 TCP 主通道（类型 90-99），数据面为独立 HTTP(S) 服务（`QHttpServer` + `QSslServer`，与主通道同一套证书与 fail-closed 口径，`--http-port`/`--http-host` 可配，基地址由登录响应的 `fileTransferBaseUrl` 下发而不是客户端猜端口）；文件字节客户端加密后上传（每文件独立 AES-256 密钥 + 分片独立 AEAD，nonce/AAD 绑定分片序号），文件名/MIME/明文大小/文件密钥只在 `FileManifest` 内随消息正文经既有 E2EE 分发，服务端只见密文与密文侧元数据；客户端 `FileTransferManager`（两遍加密上传、串行分片、失败后先查已收分片再重传、流式下载 + 整体摘要自校验、进度/取消/重试）；本地缓存**密文原样落盘**（零额外加密开销，磁盘上天然不是明文，sha256 命名 + 256 桶 + 无后缀），明文只在用户“另存为”时写出；清单含密钥，**经唯一脱敏出口 `sanitizeForUi` 拦截，四条 UI 路径（实时推送/历史翻页/离线补收/本地缓存回填）均不进 QML/JS 引擎**；QML 附件按钮与文件气泡（图标/名/大小/进度/下载/另存，编辑项对文件消息禁用）；图片消息展示内联缩略图（`data:image/jpeg;base64,`，随清单经 E2EE 到达，下载原图前即可预览）；**应用内大图查看器**（`image://xyfile/<messageId>` 由 `FileImageProvider` 从密文缓存逐片解密并在内存中解码，明文不落盘；标题带文件名与像素尺寸；footer 提供“另存为”）；**音视频元数据与应用内播放器（M8.3b）**：上传时 `MediaMetadataExtractor`（QMediaPlayer + QVideoSink）异步提取时长/分辨率/视频封面帧填入清单 `durationMs`/`width`/`height`/`thumb`（元数据缺失绝不阻断发送），气泡展示时长标签与播放按钮；播放经 `MediaPlaybackManager`（C++ QMediaPlayer + `DecryptingIODevice` 从密文缓存流式逐片解密喂给 `setSourceDevice`，明文不落盘），QML 播放器对话框控制播放/暂停/进度/音量；历史图片消息（M8.3a 前发送、清单 thumb 为空）下载后由 `localThumbnailForMessage` 本地生成缩略图并缓存到 `<cacheRoot>/thumbs/`。**M10（2026-09-14）传输体验**：上传 hashing 与另存为解密改为 GUI 线程时间片增量泵送（`QTimer(0)` 本地泵，大文件不再冻屏，密文逐字节不变）；传输横幅改 `Repeater` over 多任务列表（每任务独立进度/取消，并修复了此前横幅误作 `RowLayout` 兄弟节点挤占聊天区宽度的布局缺陷）；QML 文件状态/进度更新改 `messageId → row` Map 索引（O(1)）；文件消息不可编辑时引导“删除后重发”。**未实现**：视频动态画面渲染（QML VideoOutput 无法绑定 C++ QVideoSink，当前视频播放只输出音频轨 + 静态封面，需自定义 QSGNode）、跳重启的文件消息持久化 outbox |
...
> 2026-09-14 M10 销账（Phase 4 历史欠账清理）：① P2「上传的摘要计算在 GUI 线程同步执行」（P4.4）；② P3「`saveToFile` 在 GUI 线程同步解密」（P4.3）；③ P3「传输横幅只跟踪单个上传任务」（P4.1）；④ P3「QML `updateFileState`/`updateFileProgress` 为 O(n) 全表扫描」（P4.2）；⑤ P3「文件消息不可编辑，缺撤回重发替代路径」（P4.5）。另 §1.2「会话整表删除」由“未实现”转已实现（P3.1）。异步化采**单线程时间片增量泵送**（非工作线程）——无数据竞争、密文逐字节不变，规避了规划风险表标注的“数据损坏”并发缺陷。`TestFileTransfer` 扩至 20 用例（含 hashing/save 异步契约与调度器一致性），`ctest` 12/12、`qmllint` 零错误。
> 2026-09-14 M10 CodeReview 新增登记：本轮审查**无 P0**（最高风险的异步传输泵内存安全/密文一致性/reset 顺序经逐路径核验通过）；发现 1 项 P1（会话删除在途请求 ID 为单值，快速双删除会永久丢失首个会话的本端删除且不自愈）与 2 项 P2（`resetAuthState`/`onDisconnected` 未清 M10 新增的 typing 节流与会话删除在途状态；`LocalStore::deleteConversation` 缺事务且 `sender_keys`/`outbox` 的 `exec` 结果被静默忽略、返回值被调用方丢弃）**均已修复**（会话删除在途改 `requestId → conversationId` 多槽映射对齐 M9、两条清理路径补清 M10 状态并对在途删除上报 `conversationDeleteFailed`、本地删除加事务并检查全部 `exec`）；下表新增 2 项 P3 为审查提出但本轮**未修**的项。
...
| ~~P2~~ | ~~工程~~ | ~~上传的摘要计算在 GUI 线程同步执行~~ | ~~2026-09-11 CodeReview~~ | **已销账（2026-09-14 M10 P4.4）**：上传 hashing 改 GUI 线程时间片增量泵送（`QTimer(0)` 本地泵 + 堆分配 `HashState`，单线程无并发、密文逐字节不变）；`beginHashing` 同步生成密钥并建态后即入队返回，文件句柄首片惰性打开（源文件入队后被删也能在泵送时明确失败）。原分析：`uploadAndSend` 返回前会同步跑完第一遍流式加密 + SHA-256（为得到密文整体摘要），2 GiB 文件可阻塞界面数十秒；且此期间的 `taskProgress` 因 UI 尚未拿到 token 而全部丢弃，同步失败分支还会使上传横幅停在“准备中 0%”且无法关闭。建议拆为“同步校验返回 token + 异步 hashing”（`QTimer::singleShot(0)` 或工作线程），UI 侧改由 `tasksChanged`/`taskFailed` 维护横幅可见性 |
...
| ~~P3~~ | ~~工程~~ | ~~`saveToFile` 在 GUI 线程同步解密（最大 2 GiB）~~ | ~~2026-09-11 M8.2~~ | **已销账（2026-09-14 M10 P4.3）**：`saveToFile` 改异步返回 token（bool→QString），复用同一本地泵时间片增量解密写盘 + 传输横幅（`Repeater`）显示进度/取消，完成经 `fileSaved`、失败经 `taskFailed`；校验失败仍同步 `failTask`（与 `uploadAndSend` 同口径）。原分析：界面冻结、无进度、无法取消；建议改异步并复用现有 `taskProgress` 与横幅 |
| ~~P3~~ | ~~工程~~ | ~~传输横幅只跟踪单个上传任务~~ | ~~2026-09-11 CodeReview~~ | **已销账（2026-09-14 M10 P4.1）**：`FileTransferManager` 新增只读快照属性 `tasks`（QVariantList），横幅改 `Repeater` over 任务列表（每任务独立进度条 + 取消），并修复横幅此前误作 `RowLayout` 兄弟节点挤占聊天区一半宽度的布局缺陷（改为内容区上方全宽）。原分析：引擎支持多任务并发（`pumpNext` 遍历全部任务）但 UI 只维护 `activeUploadToken`：第二个上传的进度与失败不可见，取消也只能取消最后一个。建议横幅改为 `Repeater` over 任务列表 |
| ~~P3~~ | ~~工程~~ | ~~QML `updateFileState`/`updateFileProgress` 为 O(n) 全表扫描~~ | ~~2026-09-11 CodeReview~~ | **已销账（2026-09-14 M10 P4.2）**：`ChatView` 维护 `messageId → row` 的 JS Map（`msgModel` 变更时同步维护），两方法经 Map O(1) 定位（含命中前 `==` 校验 + 失效重建兜底，索引漂移最坏退化为性能而非正确性问题）。原分析：长会话（数千条）× 大文件（512 个分片事件）下开销可观；建议维护 `messageId -> row` 的 JS Map |
...
| P3 | 功能 | 文件消息不可编辑，缺“撤回重发”替代路径 | ~~2026-09-10 M8.1~~ | **已销账（2026-09-14 M10 P4.5）**：`MessageBubble` 文件消息的编辑菜单项禁用时显示 tooltip“文件消息不可编辑，请删除后重发”，给出替代路径（纯 UI 引导，不改服务端拒绝口径）。原分析：`processEditMessageRequest` 对 `fileId > 0` 的消息一律回 `InvalidRequest`：编辑只能改写正文而 `messages.file_id` 不变，会使清单里的 `fileId`/密钥与服务端授权、以及客户端以 `file_id` 判别清单的口径三者失配（客户端可能拿着旧 `fileId` 去申请下载票据）。正确替代应为“删除原消息 + 重发新文件消息”，属 M8.2 客户端 UI 范围（需引导与原子化），当前仅服务端拦住 |
...
- 已知限制：音视频元数据提取与播放依赖平台解码后端（Windows Media Foundation），CI/无头环境不可验证（提取失败留空、播放报错，均不崩溃）；**视频播放无动态画面**（QML VideoOutput 无法绑定 C++ QVideoSink，只输出音频轨 + 静态封面，见 §3 P3）；hashing 曾在 GUI 线程同步（大文件冻屏），已于 M10 P4.4 改时间片增量泵送销账（见 §3）。
...
- **已交付（保守主体 + 精选新能力）**：
  - **Phase 0 视觉系统与组件库地基**：Theme token 扩展（elevation/状态色/`avatarColor(id)`/字号 scale/动画曲线，清理装饰性横线注释）；SVG 图标库 + `Icon.qml`（MultiEffect 着色，替换全部 Canvas 手绘与 emoji）；基础组件库（`AppButton`/`AppTextField`/`AppDialog`/`Avatar`/`Toast`/`EmptyState`/`LoadingIndicator`/`NetworkStatusBar`）；全局反馈层（顶层 Toast + 网络状态条，失败信号全部经 Toast）。
  - **Phase 1 可用性关键路径**：加载指示器；`MainPage` 拆分为 9 个独立 Dialog（`resources/dialogs/`，`MainPage` < 500 行）；搜索体验（自动聚焦/键盘导航/`EmptyState`）。
  - **Phase 2 消息与会话体验**：`Avatar` 接入；消息气泡重构（SVG 矢量状态勾、failed 点击重发、hover 编辑/删除/复制、文本可选中）；未读分隔线 + 跳到底部 FAB（带未读徽标）；输入区多行 `TextArea`（Enter 发送/Shift+Enter 换行/字符计数器）；会话右键菜单（置顶/免打扰/标记已读/删除会话）。
  - **Phase 3 精选新协议**：会话整表删除（类型 `100-102`，硬删除 + 通知全体前成员，群聊仅群主可删，**无需 V11 迁移**）；“正在输入”指示（类型 `103-105`，服务端 fan-out + 限流 10/10s，客户端节流 4s + 5s 自动隐藏）。
  - **Phase 4 历史欠账清理**：见 §3 2026-09-14 销账（传输横幅多任务 `Repeater`、O(n)→Map 索引、`saveToFile` 异步化、上传 hashing 异步化、文件消息重发引导、Theme 装饰性注释）。
- **验收**：`ctest` 12/12（`TestFileTransfer` 扩至 20 用例，`TestNetworkManager`/`TestDatabaseManager` 各新增会话删除与 typing 用例）；`qmllint` 零错误；构建全目标通过。
- **明确排除（经用户确认，保守范围；如需另立项）**：客户端本地消息搜索、系统托盘、桌面通知、草稿、emoji 选择器、设置页、响应式/三栏布局、国际化（.ts）、群信息右侧抽屉、消息回复/引用/转发、最后在线时间、多选模式。
- **偏离规划记录**：① 协议号由规划的 `82-87` 顺延至 `100-105`（`82-89` 已被 M9 占用、`90-99` 属 M8 文件控制面）；② 会话删除无需 V11 迁移（复用既有表 + 事务内显式外键安全删除，不依赖 `PRAGMA foreign_keys`）；③ 群聊删除由规划的“仅会话成员可删”收紧为“仅群主可删”（防单成员销毁全群共享数据），通知范围由“本人所有在线设备”扩展为“全体前成员”（避免其他成员残留幽灵会话）；④ 传输横幅顺带修复了此前误作 `RowLayout` 兄弟节点挤占聊天区宽度的布局缺陷；⑤ 异步化采单线程时间片泵送而非工作线程（规避数据竞争与密文损坏风险）。
...
3. ~~**M10 搜索/通知/体验**~~（M10 UI 重构与体验完善已于 2026-09-14 完成保守范围，见 §4.2；原搜索/通知/草稿/国际化等任务经用户确认明确排除，如需另立项）。
...
XYChat_Project/
  ...
  CommonModule/           # 客户端/服务端共享模块
    protocol/             # Packet/PacketCodec（消息类型 1-105，错误码 1000-9002）、FileProtocol(M8)
  ...
  Chat-Client/
    ...
    resources/
      ...
      components/         # TitleBar/ConversationList/ChatView/MessageInput/MessageBubble/QWKButton
                          # + M10 组件库：Icon/AppButton/AppTextField/AppDialog/Avatar/Toast/
                          #   EmptyState/LoadingIndicator/NetworkStatusBar
      dialogs/            # M10: MainPage 拆出的 9 个独立对话框（Search/CreateGroup/GroupInfo/
                          #   Invite/EditMessage/ConfirmDelete/ImagePreview/MediaPlayback/SaveFile）
      theme/              # Theme.qml（darkMode 双配色 + M10 token 分层：elevation/状态色/
                          #   avatarColor(id)/字号 scale/动画曲线）
      icons/              # M10: SVG 图标库（经 Icon.qml + MultiEffect 着色，替换 Canvas 手绘与 emoji）
      main.qml
      resources.qrc
    main.cpp
  Chat-Server/
...
| 2026-09-14 | M10 完成：UI 重构与体验完善（保守范围） | 经用户确认采“保守主体（纯 QML 重构）+ 精选新能力（两项新协议）”策略，明确排除本地消息搜索/系统托盘/桌面通知/草稿/emoji/设置页/响应式布局/国际化等原 M10 任务。**Phase 0 视觉系统与组件库**：`Theme.qml` token 扩展（elevation/状态色/`avatarColor(id)` 确定性配色/字号 scale/动画曲线，并清理违反 AGENTS.md 的装饰性横线注释）；新增 `resources/icons/` SVG 图标库 + `Icon.qml`（QtQuick.Effects `MultiEffect` 单色 tint，主题切换自动重画），替换全部 Canvas 手绘与 emoji 字符图标；新增基础组件库 `AppButton`/`AppTextField`/`AppDialog`/`Avatar`/`Toast`/`EmptyState`/`LoadingIndicator`/`NetworkStatusBar`（`App` 前缀避免遮蔽 QtQuick.Controls 内置类型）；`MainWindow` 顶层接入全局 Toast + 网络状态条（消费既有 `networkManager.state`），失败信号由 `console.log` 改经 Toast。**Phase 1**：加载指示器；`MainPage`（原 1854 行含 9 个内嵌 Dialog）拆分为 `resources/dialogs/` 下 9 个独立文件（< 500 行）；搜索体验（自动聚焦/键盘导航/`EmptyState`）。**Phase 2**：`Avatar` 接入（同一用户跨位置配色一致）；消息气泡重构（emoji 状态勾→SVG 矢量勾、failed 点击重发复用 `clientMessageId` 幂等键、hover 编辑/删除/复制、正文 `Label`→只读 `TextEdit` 可选中）；未读分隔线 + 跳到底部 FAB（带未读徽标）；输入区多行 `TextArea`（Enter 发送/Shift+Enter 换行/字符计数器）；会话右键菜单新增标记已读 + 删除会话。**Phase 3 精选新协议**：① 会话整表删除——**协议号由规划 82-87 顺延至 100-105**（`DeleteConversationRequest/Response` + `ConversationDeletedNotification`，因 82-89 已被 M9 占用、90-99 属 M8）；服务端 `processDeleteConversationRequest` 成员校验 + **群聊仅群主可删**（对规划“仅成员可删”的安全性收紧）+ 事务内显式外键安全硬删除（`DatabaseManager::deleteConversation`，不依赖 `PRAGMA foreign_keys`）+ **通知全体前成员**（对规划“仅本人设备”的正确性扩展）+ `conversation_deleted` 事件离线补偿；客户端 `deleteConversation` + `conversationDeleted`/`conversationDeleteFailed` 信号 + `handleConversationDeletedNotification`（`operatorId`+`originDeviceId` 去重）+ `LocalStore::deleteConversation`；QML 右键菜单启用删除 + `ConfirmDeleteDialog` 泛化支持会话模式。**无需 V11 迁移**。② “正在输入”指示（`TypingRequest/Response/Notification` 103-105）——服务端 `processTypingRequest` 成员校验 + fan-out 到其他在线成员 + `m_typingWindow` 限流 10/10s + **不写 sync_events**；客户端 `sendTyping` C++ 侧按会话节流 4s（typing=false 不节流）+ `typingReceived` 信号；QML `MessageInput.typingActivity` → `ChatView` 头部副标题 + 1s 剪枝定时器 5s 自动隐藏。**Phase 4 历史欠账清理（销账 §3 五项）**：传输横幅改 `Repeater` over 新增只读快照属性 `FileTransferManager::tasks`（每任务独立进度/取消，并修复横幅此前误作 `RowLayout` 兄弟节点挤占聊天区一半宽度的布局缺陷，改为内容区上方全宽 + `taskProgressMap` 按 token 增量刷新 + `uploadTokens` 区分上传/下载）；`updateFileState`/`updateFileProgress` 改 `messageId → row` JS Map（O(1) + 失效重建兜底）；**`saveToFile` 与上传 hashing 异步化**——共用**单线程时间片增量泵**（`QTimer(0)` + 堆分配 `HashState`/`SaveState` 持不可拷贝的 `QFile`，每片受 8ms 预算约束后交还事件循环），`saveToFile` 签名 bool→QString token（校验失败仍同步 `failTask`），**刻意不采工作线程**以规避规划风险表标注的数据竞争/密文损坏（密文逐字节不变，`uploadThenDownloadRestoresBytesExactly` 回归锁定）；`cancelTask`/`finishTask`/`failTask`/`reset` 统一 `clearLocalStateForToken`（先关句柄再删临时文件）；文件消息编辑菜单禁用时 tooltip 引导“删除后重发”。**验证**：`TestFileTransfer` 扩至 20 用例（新增 `uploadAndSendDefersHashingToEventLoop`、`saveToFileIsAsyncWithProgress`；7 处 `saveToFile` 断言按 bool→token 迁移，同步校验失败用先连 spy 再调用口径）；`TestNetworkManager` 新增 typing 2 + 会话删除 2 用例；`TestDatabaseManager` 新增 `deleteConversationRemovesAllRelatedRows`；`ctest` **12/12** 全绿、`qmllint` 零错误、构建全目标通过。**偏离规划**：协议号 100-105（非 82-87）；会话删除无 V11 迁移；群删仅群主 + 通知全体前成员；横幅布局缺陷顺带修复；异步化用单线程泵非工作线程。文档同步：ROADMAP（§1.1 M10 行、§1.2 四能力行、§3 销账五项 + M10 销账注、§4.1 hashing 限制转已修、§4.2 重写为已交付/排除/偏离、§5 执行顺序、§6 目录树、§7 无 V11 说明、§11 本行）、PROTOCOL（状态段 + 类型表 100-105 + M10 章节）、ARCHITECTURE（客户端组件与 M10 能力边界）、SECURITY（会话删除 + typing 安全影响） |
