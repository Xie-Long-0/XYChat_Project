...
- **上传票据即时吊销**：上传完成/取消/标失败后服务端立即 `revokeFileTickets`（TTL 24 小时而分片已组装回收，持票也无处可用）。
- **下载票据滑动续期，且有绝对寿命上限**（2026-09-17 修复）：下载票据刻意允许在窗口内重复使用以支持 `Range` 分段（一次性消费与分段下载互斥），但**固定** TTL 与真实的下载时长不匹配——单次 GET 上限 4 MiB，2 GiB 文件需 2048 次串行 Range GET，慢链路必然在中途失效，而客户端会把 401 当确定性故障并丢弃全部已下载分片。现服务端在授权通过时按“最后一次使用”把寿命推回完整窗口（`DatabaseManager::renewFileTicket`），同时以 `created_at + DownloadTicketMaxLifetimeSeconds(24h)` 为**绝对上限**封顶——上限不可省：否则泄露的票据只要被持续使用就能无限续命，把“短时效”变成“永久有效”。续期只对仍然有效且未消费的票据生效（不能复活已失效票据）；续期失败不改变本次授权结论（票据此刻已验证有效），因此**不得** fail-closed 拒绝该请求。客户端侧对应地：收到 401 且已下载分片时重新申请票据并从断点续传（次数由 `MaxTicketRenewals` 单独封顶，防止“申请－再 401”活锁）。
